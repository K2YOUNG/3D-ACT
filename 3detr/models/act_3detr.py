...
MODEL_DIR = os.path.abspath(os.path.join(CUR_PATH, os.pardir))
sys.path.append(MODEL_DIR)
sys.path.append(os.path.join(MODEL_DIR, 'utils'))
...
class Model3DETR(nn.Module):
    ...
    def run_encoder(self, point_clouds):
        B, num_points, C = point_clouds.size()
    ...
    def forward(self, point_clouds):
        ...
        point_cloud_dims = [
            point_clouds.min(dim=1)[:3][0],
            point_clouds.max(dim=1)[:3][0]
        ]

        enc_pos = self.pos_embedding(enc_xyz, input_range=point_cloud_dims)

        # return: batch x npoints x channels
        return enc_features.transpose(0, 1), enc_pos.permute(2,0,1)
...
def build_3detr(args):
    ...
    # Need to build the decoder to load the pretrained weights
